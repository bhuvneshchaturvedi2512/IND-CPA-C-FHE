...
namespace seal
{
    namespace util
    {
            ...
            Ciphertext &destination)
        {
            ...
            {
            //cout << coeff_modulus[i].value() << endl; //displays value of q
            ...
            bool save_seed, Ciphertext &destination)
        {
            ...
            if (is_ntt_form || !save_seed)
            {
                ...
                /*vector_a = fopen("vector_a_ntt.txt","a");
    	    	for(int i = 0; i < coeff_count; i++)
    	    	{
    		    fprintf(vector_a, "%lu ", c1[i]);
    	    	}
    	    	fprintf(vector_a, "\n");
    	    	fclose(vector_a);*/
            }
            ...
            vector_e = fopen("vector_e_original.txt","a");
    	    for(int i = 0; i < coeff_count; i++)
    	    {
    	        if(noise.get()[i] < 100)
    		    fprintf(vector_e, "%ld ", noise.get()[i]);
    		else
    		    fprintf(vector_e, "%ld ", (noise.get()[i] - 132120577));
    	    }
    	    fprintf(vector_e, "\n");
    	    fclose(vector_e);
            ...
            { 
                ...
                /*vector_as_ntt = fopen("vector_as_ntt.txt","a");
    	        for(int i = 0; i < coeff_count; i++)
    	        {
    		    fprintf(vector_as_ntt, "%lu ", c0[i]);
    	        }
    	        fprintf(vector_as_ntt, "\n");
    	        fclose(vector_as_ntt);*/
    	        
                if (is_ntt_form)
                {
                    // Transform the noise e into NTT representation
                    ntt_negacyclic_harvey(noise.get() + i * coeff_count, ntt_tables[i]);
                    /*vector_e = fopen("vector_e_ntt.txt","a");
    	    	    for(int i = 0; i < coeff_count; i++)
    	    	    {
    			fprintf(vector_e, "%lu ", noise.get()[i]);
    	    	    }
    	    	    fprintf(vector_e, "\n");
    	    	    fclose(vector_e);*/
                }
                ...
                {
                    // Transform the product as into NTT representation
                    inverse_ntt_negacyclic_harvey(c0 + i * coeff_count, ntt_tables[i]);
                    /*vector_as_inv_ntt = fopen("vector_as_inv_ntt.txt","a");
    	    	    for(int i = 0; i < coeff_count; i++)
    	    	    {
    			fprintf(vector_as_inv_ntt, "%lu ", c0[i]);
    	    	    }
    	    	    fprintf(vector_as_inv_ntt, "\n");
    	    	    fclose(vector_as_inv_ntt);*/
                }
                ...
                /*vector_as_e = fopen("vector_as_e_original.txt","a");
    	        for(int i = 0; i < coeff_count; i++)
    	        {
    		    fprintf(vector_as_e, "%lu ", c0[i]);
    	        }
    	        fprintf(vector_as_e, "\n");
    	        fclose(vector_as_e);*/
                ...
                /*vector_as_e = fopen("vector_as_e_negated.txt","a");
    	        for(int i = 0; i < coeff_count; i++)
    	        {
    		    fprintf(vector_as_e, "%lu ", c0[i]);
    	        }
            ...
            if (!is_ntt_form && !save_seed)
            {
                ...
                /*vector_a = fopen("vector_a_inv_ntt.txt","a");
    	    	for(int i = 0; i < coeff_count; i++)
    	    	{
    		    fprintf(vector_a, "%lu ", c1[i]);
    	    	}
